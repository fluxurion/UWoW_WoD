...
void Battlenet::Session::HandleLogonRequest(Authentication::LogonRequest3 const& logonRequest)
{
    if (_queryCallback)
    {
        ...
        sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::LogonRequest] %s attempted to log too quick after previous attempt!", GetClientInfo().c_str());
        return;
    }
    ...
    if (logonRequest.Common.Program != "WoW")
    {
        ...
        AsyncWrite(logonResponse);
        sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::LogonRequest] %s attempted to log in with game other than WoW (using %s)!", GetClientInfo().c_str(), logonRequest.Common.Program.c_str());
        return;
    }
    ...
    if (!sComponentMgr->HasPlatform(logonRequest.Common.Platform))
    {
        ...
        AsyncWrite(logonResponse);
        sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::LogonRequest] %s attempted to log in from an unsupported platform (using %s)!", GetClientInfo().c_str(), logonRequest.Common.Platform.c_str());
        return;
    }
    ...
    if (!sComponentMgr->HasPlatform(logonRequest.Common.Locale))
    {
        ...
        AsyncWrite(logonResponse);
        sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::LogonRequest] %s attempted to log in with unsupported locale (using %s)!", GetClientInfo().c_str(), logonRequest.Common.Locale.c_str());
        return;
    }
    ...
    for (Version::Record const& component : logonRequest.Common.Versions)
    {
        if (!sComponentMgr->HasComponent(&component))
        {
            ...
            {
                logonResponse->SetAuthResult(AUTH_INVALID_PROGRAM);
                sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::LogonRequest] %s is using unsupported component program %s!", GetClientInfo().c_str(), component.ProgramId.c_str());
            }
            else if (!sComponentMgr->HasPlatform(component.Component))
            {
                logonResponse->SetAuthResult(AUTH_INVALID_OS);
                sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::LogonRequest] %s is using unsupported component platform %s!", GetClientInfo().c_str(), component.Component.c_str());
            }
            else
            {
...
void Battlenet::Session::HandleLogonRequestCallback(PreparedQueryResult result)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::LogonRequest] Account '%s' is locked to IP - '%s' is logging in from '%s'", _accountInfo->Login.c_str(), _accountInfo->LastIP.c_str(), ip_address.c_str());
    ...
    {
        sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::LogonRequest] Account '%s' is not locked to ip", _accountInfo->Login.c_str());
        if (_accountInfo->LockCountry.empty() || _accountInfo->LockCountry == "00")
            sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::LogonRequest] Account '%s' is not locked to country", _accountInfo->Login.c_str());
        else if (!_accountInfo->LockCountry.empty() && !_ipCountry.empty())
        {
            sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::LogonRequest] Account '%s' is locked to country: '%s' Player country is '%s'", _accountInfo->Login.c_str(), _accountInfo->LockCountry.c_str(), _ipCountry.c_str());
    ...
    if (_accountInfo->IsBanned)
    {
        if (_accountInfo->IsPermanenetlyBanned)
        {
            ...
            AsyncWrite(logonResponse);
            sLog->outDebug(LOG_FILTER_BNET_SESSION, "'%s:%d' [Battlenet::LogonRequest] Banned account %s tried to login!", ip_address.c_str(), GetRemotePort(), _accountInfo->Login.c_str());
            return;
        }
        else
        {
            ...
            AsyncWrite(logonResponse);
            sLog->outDebug(LOG_FILTER_BNET_SESSION, "'%s:%d' [Battlenet::LogonRequest] Temporarily banned account %s tried to login!", ip_address.c_str(), GetRemotePort(), _accountInfo->Login.c_str());
            return;
        }
    }
...
void Battlenet::Session::HandleResumeRequest(Authentication::ResumeRequest const& resumeRequest)
{
    if (_queryCallback)
    {
        ...
        logonResponse->SetAuthResult(AUTH_LOGON_TOO_FAST);
        AsyncWrite(logonResponse);
        sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::ResumeRequest] %s attempted to log too quick after previous attempt!", GetClientInfo().c_str());
        return;
    }
...
{
    if (sLog->ShouldLog(LOG_FILTER_BNET_SESSION, LOG_LEVEL_TRACE))
...
void Battlenet::Session::ReadHandler()
{
    ...
    while (!stream.IsRead())
    {
        try
        {
            ...
            if (ClientPacket* packet = sPacketManager.CreateClientPacket(header, stream))
            {
                if (sPacketManager.IsHandled(header))
                    sLog->outDebug(LOG_FILTER_BNET_SESSION, "%s Received %s", GetClientInfo().c_str(), PacketToStringHelper(packet).c_str());
            ...
            {
                sLog->outDebug(LOG_FILTER_BNET_SESSION, "%s Received unknown %s", GetClientInfo().c_str(), header.ToString().c_str());
                break;
            }
        ...
        {
            sLog->outError(LOG_FILTER_BNET_SESSION, "%s Exception thrown during packet processing %s", GetClientInfo().c_str(), e.what());
            CloseSocket();
            return;
        }
    }
...
{
    std::string ip_address = GetRemoteIpAddress().to_string();
    sLog->outTrace(LOG_FILTER_BNET_SESSION, "Accepted connection from %s", ip_address.c_str());
    ...
    if (_queryCallback)
    {
        ...
        logonResponse->SetAuthResult(AUTH_LOGON_TOO_FAST);
        AsyncWrite(logonResponse);
        sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Session::Start] %s attempted to log too quick after previous attempt!", GetClientInfo().c_str());
        return;
    }
...
void Battlenet::Session::CheckIpCallback(PreparedQueryResult result)
{
    if (result)
    {
        ...
        if (banned)
        {
            ...
            AsyncWrite(logonResponse);
            sLog->outDebug(LOG_FILTER_BNET_SESSION, "[Battlenet::LogonRequest] Banned ip '%s:%d' tries to login!", GetRemoteIpAddress().to_string().c_str(), GetRemotePort());
            return;
        }
    }
...
void Battlenet::Session::AsyncWrite(ServerPacket* packet)
{
    ...
    sLog->outDebug(LOG_FILTER_BNET_SESSION, "%s Sending %s", GetClientInfo().c_str(), PacketToStringHelper(packet).c_str());
...
bool Battlenet::Session::HandlePasswordModule(BitStream* dataStream, ServerPacket** response)
{
    ...
    else
    {
        ...
        if (_gameAccountInfo->IsBanned)
        {
            ...
            {
                logonResponse->SetAuthResult(LOGIN_BANNED);
                sLog->outDebug(LOG_FILTER_BNET_SESSION, "'%s:%d' [Battlenet::Password] Banned account %s tried to login!", GetRemoteIpAddress().to_string().c_str(), GetRemotePort(), _accountInfo->Login.c_str());
            }
            else
            {
                logonResponse->SetAuthResult(LOGIN_SUSPENDED);
                sLog->outDebug(LOG_FILTER_BNET_SESSION, "'%s:%d' [Battlenet::Password] Temporarily banned account %s tried to login!", GetRemoteIpAddress().to_string().c_str(), GetRemotePort(), _accountInfo->Login.c_str());
            }
...
bool Battlenet::Session::HandleSelectGameAccountModule(BitStream* dataStream, ServerPacket** response)
{
    ...
    if (_gameAccountInfo->IsBanned)
    {
        ...
        {
            logonResponse->SetAuthResult(LOGIN_BANNED);
            sLog->outDebug(LOG_FILTER_BNET_SESSION, "'%s:%d' [Battlenet::SelectGameAccount] Banned account %s tried to login!", GetRemoteIpAddress().to_string().c_str(), GetRemotePort(), _accountInfo->Login.c_str());
        }
        else
        {
            logonResponse->SetAuthResult(LOGIN_SUSPENDED);
            sLog->outDebug(LOG_FILTER_BNET_SESSION, "'%s:%d' [Battlenet::SelectGameAccount] Temporarily banned account %s tried to login!", GetRemoteIpAddress().to_string().c_str(), GetRemotePort(), _accountInfo->Login.c_str());
        }
...
{
    sLog->outError(LOG_FILTER_BNET_SESSION, "Unhandled module.");
    Authentication::LogonResponse* logonResponse = new Authentication::LogonResponse();
    logonResponse->SetAuthResult(AUTH_CORRUPTED_MODULE);
    ReplaceResponse(response, logonResponse);
    return false;
