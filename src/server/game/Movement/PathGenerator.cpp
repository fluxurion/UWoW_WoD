    ...
    _navMeshQuery(NULL)
{
    ...
    sLog->outDebug(LOG_FILTER_MAPS, "++ PathGenerator::PathGenerator for %s", _sourceUnit->GetGUID().ToString().c_str());
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, bool straightLine)
{
    ...
    sLog->outDebug(LOG_FILTER_MAPS, "++ PathGenerator::CalculatePath() for %s", _sourceUnit->GetGUID().ToString().c_str());
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (!suffixPolyLength || dtStatusFailed(dtResult))
        {
            ...
            sLog->outError(LOG_FILTER_MAPS, "%s's Path Build failed: 0 length path", _sourceUnit->GetGUID().ToString().c_str());
        }
