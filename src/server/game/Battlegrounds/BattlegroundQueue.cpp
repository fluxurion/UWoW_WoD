...
void BattlegroundQueue::RemovePlayer(ObjectGuid guid, bool decreaseInvitedCount)
{
    ...
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundQueue: couldn't find player to remove GUID: %u", guid.GetCounter());
        return;
    }
    ...
    for (int32 bracket_id_tmp = MAX_BATTLEGROUND_BRACKETS - 1; bracket_id_tmp >= 0 && bracket_id == -1; --bracket_id_tmp)
    {
        ...
        for (uint32 j = index; j < BG_QUEUE_MAX; j += MAX_TEAMS)
        {
    ...
    if (bracket_id == -1)
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "BattlegroundQueue: ERROR Cannot find groupinfo for player GUID: %u", guid.GetCounter());
        return;
    }
    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "BattlegroundQueue: Removing player GUID %u, from bracket_id %u", guid.GetCounter(), (uint32)bracket_id);
    ...
    if (group->IsInvitedToBGInstanceGUID && group->IsRated && decreaseInvitedCount)
    {
        ...
        {
            if (Bracket* bracket = player->getBracket(BattlegroundMgr::BracketByJoinType(group->JoinType)))
    ...
    else if (!group->IsInvitedToBGInstanceGUID && group->IsRated)
    {
        ...
        if (Player* plr2 = ObjectAccessor::FindPlayer(group->Players.begin()->first))
        {
            ...
            sBattlegroundMgr->BuildBattlegroundStatusNone(&battlefieldStatus, plr2, queueSlot, 0, bg, group->JoinType);
...
bool BattlegroundQueue::InviteGroupToBG(GroupQueueInfo* ginfo, Battleground* bg, uint32 side)
{
    ...
    if (!ginfo->IsInvitedToBGInstanceGUID)
    {
        ...
        for (std::map<ObjectGuid, PlayerQueueInfo*>::iterator itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
        {
            ...
            m_events.AddEvent(removeEvent, m_events.CalculateTime(bg->isArena() ? ARENA_INVITE_ACCEPT_WAIT_TIME : BG_INVITE_ACCEPT_WAIT_TIME));

            uint32 queueSlot = player->GetBattlegroundQueueIndex(bgQueueTypeId);

            sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Battleground: invited player %s (%u) to BG instance %u queueindex %u bgtype %u, I can't help it if they don't press the enter battle button.", player->GetName(), player->GetGUID().GetCounter(), bg->GetInstanceID(), queueSlot, bg->GetTypeID());

            WorldPackets::Battleground::BattlefieldStatusNeedConfirmation battlefieldStatus;
            sBattlegroundMgr->BuildBattlegroundStatusNeedConfirmation(&battlefieldStatus, bg, player, queueSlot, player->GetBattlegroundQueueJoinTime(bgTypeId), bg->isArena() ? ARENA_INVITE_ACCEPT_WAIT_TIME : BG_INVITE_ACCEPT_WAIT_TIME, ginfo->JoinType);
...
BattlegroundTypeId BattlegroundQueue::GenerateRandomMap(BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id)
{
    ...
    if (selectionWeights)
    {
        ...
        for (uint32 i = TEAM_ALLIANCE; i < MAX_TEAMS; i++)
        {
            ...
            {
                for (int8 j = TEAM_ALLIANCE; j < MAX_TEAMS; ++j)
        ...
        for (BattlegroundSelectionWeightMap::const_iterator it = selectionWeights->begin(); it != selectionWeights->end(); ++it)
        {
            ...
            if (!bl)
                continue;
            if (bgTypeId == BATTLEGROUND_RATED_10_VS_10 && bl->RatedPlayers != 10)
                continue;
        ...
        if (found == 2)
        {
            ...
            if (arena->isRated())
            {
                ...
                switch (arena->GetJoinType())
                {
                    ...
                    default:
                        sLog->outArena("Arena match type: %u for Team1Id: %u - Team2Id: %u started.", arena->GetJoinType(), arena->GetGroupIdByIndex(TEAM_ALLIANCE), arena->GetGroupIdByIndex(TEAM_HORDE));
                        break;
                }
            }
        }
...
bool BGQueueInviteEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue or in battleground
    {
        ...
        {
            WorldPackets::Battleground::BattlefieldStatusNeedConfirmation battlefieldStatus;
            sBattlegroundMgr->BuildBattlegroundStatusNeedConfirmation(&battlefieldStatus, bg, player, queueSlot, player->GetBattlegroundQueueJoinTime(m_BgTypeId), (bg->isArena() ? ARENA_INVITE_ACCEPT_WAIT_TIME : BG_INVITE_ACCEPT_WAIT_TIME) - INVITATION_REMIND_TIME, 0);
            player->SendDirectMessage(battlefieldStatus.Write());
        }
    }
...
bool BGQueueRemoveEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue, or in Battleground
    {
        ...
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Battleground: removing player %u from bg queue for instance %u because of not pressing enter battle in time.", player->GetGUID().GetCounter(), m_BgInstanceGUID);
            ...
            WorldPackets::Battleground::BattlefieldStatusNone battlefieldStatus;
            sBattlegroundMgr->BuildBattlegroundStatusNone(&battlefieldStatus, player, queueSlot, player->GetBattlegroundQueueJoinTime(m_BgTypeId), bg, 0);
            player->SendDirectMessage(battlefieldStatus.Write());
