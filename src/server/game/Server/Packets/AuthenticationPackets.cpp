...
WorldPacket const* WorldPackets::Auth::AuthResponse::Write()
{
    ...
    if (SuccessInfo)
    {
        ...
        _worldPacket << static_cast<uint32>(SuccessInfo->VirtualRealms.size());
        ...
        _worldPacket << static_cast<uint32>(SuccessInfo->AvailableRaces->size());
        _worldPacket << static_cast<uint32>(SuccessInfo->AvailableClasses->size());
        _worldPacket << static_cast<uint32>(SuccessInfo->Templates.size());
        _worldPacket << uint32(SuccessInfo->CurrencyID);

        for (auto const& realm : SuccessInfo->VirtualRealms)
        {
        ...
        {
            _worldPacket << uint8(race.first);
            _worldPacket << uint8(race.second);
        }

        for (auto const& klass : *SuccessInfo->AvailableClasses)
        {
            _worldPacket << uint8(klass.first);
            _worldPacket << uint8(klass.second);
        }
