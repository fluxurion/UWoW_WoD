...
bool ChatHandler::SetDataForCommandInTable(ChatCommand* table, const char* text, uint32 security, std::string const& help, std::string const& fullcommand)
{
    ...
    for (uint32 i = 0; table[i].Name != NULL; i++)
    {
        ...
        if (table[i].SecurityLevel != security)
            sLog->outDebug(LOG_FILTER_GENERAL, "Table `command` overwrite for command '%s' default security (%u) by %u", fullcommand.c_str(), table[i].SecurityLevel, security);
...
void ChatHandler::FillMessageData(WorldPacket* data, WorldSession* session, uint8 type, uint32 language, const char *channelName, ObjectGuid target_guid, const char *message, Unit* speaker, const char* addonPrefix /*= NULL*/)
{
    ...
    c.realmId = (speaker && speaker->GetTypeId() == TYPEID_PLAYER) ? GetVirtualRealmAddress() : 0;
    ...
    switch (type)
    {
        ...
        default:
            if (type != CHAT_MSG_WHISPER_INFORM && type != CHAT_MSG_IGNORED && type != CHAT_MSG_DND && type != CHAT_MSG_AFK)
                c.targetGuid = ObjectGuid::Empty;               // only for CHAT_MSG_WHISPER_INFORM used original value target_guid
            break;
    }
