...
void WorldSession::HandlePetitionBuy(WorldPackets::Petition::PetitionBuy& packet)
{
    ...
    if (sObjectMgr->IsReservedName(packet.Title) || !ObjectMgr::IsValidCharterName(packet.Title) ||
        (sWorld->getBoolConfig(CONFIG_WORD_FILTER_ENABLE) && !sWordFilterMgr->FindBadWord(packet.Title).empty()))
    {
    ...
    if (!pProto)
    {
        player->SendBuyError(BUY_ERR_CANT_FIND_ITEM, NULL, charterid, 0);
        return;
    }

    if (!player->HasEnoughMoney(uint64(cost)))
    {
        player->SendBuyError(BUY_ERR_NOT_ENOUGHT_MONEY, creature, charterid, 0);
        return;
    }
    ...
    if (msg != EQUIP_ERR_OK)
    {
        player->SendEquipError(msg, NULL, NULL, charterid);
        return;
    }
...
void WorldSession::HandlePetitionRenameGuild(WorldPackets::Petition::PetitionRenameGuild& packet)
{
    ...
    if (sObjectMgr->IsReservedName(packet.NewGuildName) || !ObjectMgr::IsValidCharterName(packet.NewGuildName) ||
        (sWorld->getBoolConfig(CONFIG_WORD_FILTER_ENABLE) && !sWordFilterMgr->FindBadWord(packet.NewGuildName).empty()))
    {
