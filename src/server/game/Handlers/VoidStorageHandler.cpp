...
void WorldSession::HandleVoidStorageTransfer(WorldPackets::VoidStorage::VoidStorageTransfer& packet)
{
    ...
    int64 cost = uint64(packet.Deposits.size() * VOID_STORAGE_STORE_ITEM * auraCostMod);
    ...
    for (size_t i = 0; i < packet.Withdrawals.size(); ++i)
    {
        ...
        Item* item = player->StoreNewItem(dest, itemVS->ItemEntry, true, itemVS->ItemRandomPropertyId, GuidSet(), itemVS->BonusListIDs);
