...
void PetAI::_stopAttack()
{
    if (!me->isAlive())
    {
        sLog->outDebug(LOG_FILTER_PETS, "Creature stoped attacking cuz his dead [guid=%s]", me->GetGUID().ToString().c_str());
        me->GetMotionMaster()->Clear();
...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    else if (me->getVictim())
    {
        ...
        if (_needToStop())
        {
            sLog->outDebug(LOG_FILTER_PETS, "Pet AI stopped attacking [guid=%s]", me->GetGUID().ToString().c_str());
            _stopAttack();
            return;
        }
