...
void Spell::EffectTriggerMissileSpell(SpellEffIndex effIndex)
{
    ...
    //m_spellInfo->Id, triggered_spell_id, effectHandleMode, unitTarget ? unitTarget->GetGUID() : 0, m_spellInfo->GetEffect(effIndex, m_diffMode)->GetProvidedTargetMask(), GetTargetCount());
...
void Spell::EffectUnlearnSpecialization(SpellEffIndex effIndex)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Spell: Player %u has unlearned spell %u from NpcGUID: %u", player->GetGUID().GetCounter(), spellToUnlearn, m_caster->GetGUID().GetCounter());
}
    ...
    if (gameObjTarget)
    {
        ...
        if (!gameObjTarget->isSpawned() && !player->isGameMaster())
        {
            sLog->outError(LOG_FILTER_SPELLS_AURAS, "Possible hacking attempt: Player %s [guid: %u] tried to loot a gameobject [entry: %u id: %u] which is on respawn time without being in GM mode!",
                           player->GetName(), player->GetGUID().GetCounter(), gameObjTarget->GetEntry(), gameObjTarget->GetGUID().GetCounter());
            return;
        }
...
void Spell::EffectStuck(SpellEffIndex /*effIndex*/)
{
    ...
    sLog->outInfo(LOG_FILTER_SPELLS_AURAS, "Player %s (guid %u) used auto-unstuck future at map %u (%f, %f, %f)", target->GetName(), target->GetGUID().GetCounter(), m_caster->GetMapId(), m_caster->GetPositionX(), target->GetPositionY(), target->GetPositionZ());
...
void Spell::EffectApplyGlyph(SpellEffIndex effIndex)
{
    ...
    if (m_caster->GetTypeId() != TYPEID_PLAYER || m_misc.GlyphSlot >= MAX_GLYPH_SLOT_INDEX)
        return;
    ...
    if (uint32 glyph = m_spellInfo->GetEffect(effIndex, m_diffMode)->MiscValue)
    {
        ...
        {
            if (GlyphSlotEntry const* gs = sGlyphSlotStore.LookupEntry(player->GetGlyphSlot(m_misc.GlyphSlot)))
            {
            ...
            // remove old glyph
            if (uint32 oldglyph = player->GetGlyph(player->GetActiveSpec(), m_misc.GlyphSlot))
            {
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const* properties, uint32 numGuardians)
{
    ...
    for (uint32 count = 0; count < numGuardians; ++count)
    {
        ...
        {
            if (uint32 weapon = m_caster->GetUInt32Value(PLAYER_FIELD_VISIBLE_ITEMS + EQUIPMENT_SLOT_MAINHAND * 3))
            {
...
void Spell::EffectCreateHeirloomItem(SpellEffIndex effIndex)
{
    ...
    bonusList.push_back(collectionMgr->GetHeirloomBonus(m_misc.Data));

    DoCreateItem(effIndex, m_misc.Data, bonusList);
    ExecuteLogEffectCreateItem(effIndex, m_misc.Data);
}
...
void Spell::EffectUpgradeHeirloom(SpellEffIndex /*effIndex*/)
{
    ...
    CollectionMgr* collectionMgr = m_caster->ToPlayer()->GetCollectionMgr();
    if (collectionMgr)
        collectionMgr->UpgradeHeirloom(m_misc.Data, m_castItemEntry);
}
