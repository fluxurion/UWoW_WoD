...
void AuraApplication::_HandleEffect(uint8 effIndex, bool apply)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "AuraApplication::_HandleEffect: GetId %i, GetAuraType %u, apply: %u: amount: %i, m_send_baseAmount: %i, effIndex: %i GetDuration %i, guid %u",
    GetBase()->GetId(), aurEff->GetAuraType(), apply, aurEff->GetAmount(), aurEff->GetBaseSendAmount(), effIndex, GetBase()->GetDuration(), GetBase()->GetOwner()->GetGUIDLow());
...
Aura* Aura::TryRefreshStackOrCreate(SpellInfo const* spellproto, uint32 tryEffMask, WorldObject* owner, Unit* caster, int32* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, ObjectGuid casterGUID /*= 0*/, bool* refresh /*= NULL*/, uint16 stackAmount, Spell* spell)
{
    ASSERT(spellproto);
    ASSERT(owner);
...
Aura* Aura::Create(SpellInfo const* spellproto, uint32 effMask, WorldObject* owner, Unit* caster, int32* baseAmount, Item* castItem, ObjectGuid casterGUID, uint16 stackAmount, Spell* spell)
{
    ASSERT(effMask);
    ASSERT(spellproto);
    ASSERT(owner);
...
void Aura::_UnapplyForTarget(Unit* target, Unit* caster, AuraApplication * auraApp)
{
    ...
    {
        sLog->outError(LOG_FILTER_SPELLS_AURAS, "Aura::_UnapplyForTarget, target:%u, caster:%u, spell:%u was not found in owners application map! GetGUID %s",
        target->GetGUIDLow(), caster ? caster->GetGUIDLow() : 0, auraApp->GetBase()->GetSpellInfo()->Id, target->GetGUID().ToString().c_str());
        //ASSERT(false);
        return;
    }
...
void Aura::Update(uint32 diff, Unit* caster)
{
    if (m_duration > 0 || (!IsPassive() && m_duration == -1))
    {
        ...
        // handle PowerCostPerSecond/PowerCostPercentagePerSecond
        if (m_timeCla)
        {
            ...
            case SPELLFAMILY_ROGUE:
            {
                ...
                switch (GetId())
                {
                    ...
                    case 2094: // Blind
                    {
                        ...
                        {
                            target->RemoveAurasByType(SPELL_AURA_PERIODIC_DAMAGE, ObjectGuid::Empty, target->GetAura(32409));
