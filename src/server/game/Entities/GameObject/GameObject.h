...
struct GameObjectTemplate
{
    ...
    bool IsDespawnAtAction() const
    {
        switch (type)
        {
            ...
            case GAMEOBJECT_TYPE_GOOBER: return goober.consumable != 0 || goober.cooldown != 0;
...
class GameObject : public WorldObject, public GridObject<GameObject>
{
    public:
        ...
        GameobjectTypes GetGoType() const { return GameobjectTypes(GetByteValue(GAMEOBJECT_FIELD_BYTES_1, 1)); }
        void SetGoType(GameobjectTypes type) { SetByteValue(GAMEOBJECT_FIELD_BYTES_1, 1, type); }
        GOState GetGoState() const { return GOState(GetByteValue(GAMEOBJECT_FIELD_BYTES_1, 0)); }
        void SetGoState(GOState state);
        uint32 CalculateAnimDuration(GOState oldState, GOState newState) const;
        uint8 GetGoArtKit() const { return GetByteValue(GAMEOBJECT_FIELD_BYTES_1, 2); }
        void SetGoArtKit(uint8 artkit);
        uint8 GetGoAnimProgress() const { return GetByteValue(GAMEOBJECT_FIELD_BYTES_1, 3); }
        void SetGoAnimProgress(uint8 animprogress) { SetByteValue(GAMEOBJECT_FIELD_BYTES_1, 3, animprogress); }
