...
class Unit : public WorldObject
{
    public:
        ...
        float GetStat(Stats stat) const { return float(GetInt32Value(UNIT_FIELD_STATS+stat)); }
        ...
        int32 InterceptionOfDamage(Unit* victim, uint32 damage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellInfo = NULL);
        ...
        int32 CalculateMonkMeleeAttacks(float coeff);
        ...
        bool isWildBattlePet()      const { return HasFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_WILD_BATTLE_PET); }
        ...
        void CastSpell(Position pos, uint32 spellId, bool triggered, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, ObjectGuid originalCaster = ObjectGuid::Empty);
        void CastSpell(float x, float y, float z, uint32 spellId, TriggerCastFlags triggeredCastFlags = TRIGGERED_NONE, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, ObjectGuid originalCaster = ObjectGuid::Empty);
        ...
        void CastCustomSpell(uint32 spellId, SpellValueMod mod, int32 value, Unit* Victim = NULL, bool triggered = true, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, ObjectGuid originalCaster = ObjectGuid::Empty);
        void CastCustomSpell(uint32 spellId, CustomSpellValues const &value, Unit* Victim = NULL, bool triggered = true, Item* castItem = NULL, AuraEffect const* triggeredByAura = NULL, ObjectGuid originalCaster = ObjectGuid::Empty);
        ...
        */
        ...
        {
            ShapeshiftForm form = GetShapeshiftForm();
            return form != FORM_NONE && form != FORM_BATTLESTANCE && form != FORM_BERSERKERSTANCE && form != FORM_DEFENSIVESTANCE && form != FORM_GLADIATORSTANCE &&
        ...
        void SetPhaseId(std::set<uint32> const& newPhase, bool update);// overwrite WorldObject::SetPhaseId
        ...
        GuidList* GetSaveThreatList() { return &m_savethreatlist; }
        void ClearSaveThreatTarget() { m_savethreatlist.clear(); }
        uint32 GetSizeSaveThreat() { return m_savethreatlist.size(); }
        void AddThreatTarget(ObjectGuid const& targetGuid) { m_savethreatlist.push_back(targetGuid); }
        void RemoveThreatTarget(ObjectGuid const& targetGuid) { m_savethreatlist.remove(targetGuid); }
        ...
        Unit* GetMisdirectionTarget() { return !m_misdirectionTargetGUID.IsEmpty() ? GetUnit(*this, m_misdirectionTargetGUID) : NULL; }
