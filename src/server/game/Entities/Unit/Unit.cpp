...
void Unit::CastSpell(Position pos, uint32 spellId, bool triggered, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster)
{
    ...
    SpellCastTargets targets;
    targets.SetDst(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), GetOrientation());
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(const Unit* victim, WeaponAttackType attType) const
{
    ...
    float dodge_chance = victim->GetUnitDodgeChanceAgainst(this);
    float block_chance = victim->GetUnitBlockChanceAgainst(this);
    float parry_chance = victim->GetUnitParryChanceAgainst(this);
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    bool canDodge = true;
    bool canParry = true;
    bool canBlock = (spell->AttributesEx3 & SPELL_ATTR3_BLOCKABLE_SPELL) != 0;
...
void Unit::SendSpellNonMeleeDamageLog(SpellNonMeleeDamage* log)
{
    ...
    if (newDamage > log->preHitHealth)
        packet.Overkill = newDamage - log->preHitHealth;
    else
        packet.Overkill = 0;
            ...
            switch (dummySpell->Id)
            {
                ...
                case 134732: // Battle Fatigue
                {
                    if (Player* plr = ToPlayer())
                        if ((plr->InArena() || plr->InRBG()) && HasAura(SPELL_BATTLE_FATIGUE))
                            return false;
            ...
            switch (dummySpell->Id)
            {
                case 108370: // Soul Leech
                {
                    if (Player * warlock = ToPlayer())
                    {
                        ...
                        {
                            int32 bp0 = basepoints0 / 2;
                            CastCustomSpell(pet, triggered_spell_id, &bp0, NULL, NULL, true);
                        }
                    }
                    break;
                }
...
Minion *Unit::GetFirstMinion() const
{
    if (ObjectGuid pet_guid = GetMinionGUID())
    {
        ...
        sLog->outError(LOG_FILTER_UNITS, "Unit::GetFirstMinion: Minion %u not exist.", pet_guid.GetGUIDLow());
...
Unit* Unit::GetCharm() const
{
    if (ObjectGuid charm_guid = GetCharmGUID())
    {
        ...
        sLog->outError(LOG_FILTER_UNITS, "Unit::GetCharm: Charmed creature %u not exist.", charm_guid.GetGUIDLow());
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        // ... and attack power
        AuraEffectList mHealingDonebyAP = GetAuraEffectsByType(SPELL_AURA_MOD_SPELL_HEALING_OF_ATTACK_POWER);
...
{
    uint32 mask = 0;
    SpellImmuneList mechanicList = m_spellImmune[IMMUNITY_SCHOOL];
    for (SpellImmuneList::const_iterator itr = mechanicList.begin(); itr != mechanicList.end(); ++itr)
        mask |= itr->type;
...
int32 Unit::ModifyHealth(int32 dVal)
{
    ...
    if(dVal < 0)
    {
        AuraEffectList mTotalAuraList = GetAuraEffectsByType(SPELL_AURA_PROC_ON_HP_BELOW);
        for (AuraEffectList::const_iterator i = mTotalAuraList.begin(), next; i != mTotalAuraList.end(); i = next)
        {
            next = i;
            ++next;
...
{
    for (GuidList::const_iterator itr = m_savethreatlist.begin(); itr != m_savethreatlist.end(); ++itr)
        if ((*itr) == targetGuid)
            return true;
...
int32 Unit::CalculateSpellDamage(Unit const* target, SpellInfo const* spellProto, uint8 effect_index, int32 const* basePoints, Item* m_castitem, bool lockBasePoints, float* variance /*= nullptr*/) const
{
    if (variance)
        *variance = 0.0f;
...
void Unit::InitialPowers(bool maxpower)
{
    ...
    SendMessageToSet(packet.Write(), GetTypeId() == TYPEID_PLAYER ? true : false);
}
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, DamageInfo* dmgInfoProc, SpellInfo const* procAura, std::list<uint32>* mSpellModsList)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && procSpell && (procSpell->Id == 348 || procSpell->Id == 157736 || procSpell->Id == 108686) && procExtra & PROC_EX_CRITICAL_HIT)
...
void Unit::SendDurabilityLoss(Player* receiver, uint32 percent)
{
    ...
    receiver->GetSession()->SendPacket(durabilityDamage.Write());
}
...
Aura* Unit::AddAura(SpellInfo const* spellInfo, uint32 effMask, Unit* target, Item* castItem, uint16 stackAmount)
{
    ...
    if (Aura* aura = Aura::TryRefreshStackOrCreate(spellInfo, effMask, target, this, NULL, castItem, ObjectGuid::Empty, NULL, stackAmount))
    {
...
uint32 Unit::GetCombatRatingDamageReduction(CombatRating cr, float cap, uint32 damage) const
{
    ...
    if (cr == CR_RESILIENCE_PLAYER_DAMAGE_TAKEN || cr == CR_RESILIENCE_CRIT_TAKEN)
    {
        if (Player* pl = GetCharmerOrOwnerPlayerOrPlayerItself())
        {
            ...
            uint32 resRating  = float(pl->GetUInt32Value(PLAYER_FIELD_COMBAT_RATINGS + CR_RESILIENCE_PLAYER_DAMAGE_TAKEN));
...
void Unit::SendSpellCooldown(int32 spellId, int32 spell_cooldown, int32 cooldown)
{
    ...
    player->GetSession()->SendPacket(cooldowns.Write());
}
...
{
    if (!object->InSamePhase(i_source))
        return false;
...
SpellInfo const* Unit::GetCastSpellInfo(SpellInfo const* spellInfo) const
{
    ...
    Unit::AuraEffectList swaps2 = GetAuraEffectsByType(SPELL_AURA_OVERRIDE_ACTIONBAR_SPELLS_2);
    if (!swaps2.empty())
