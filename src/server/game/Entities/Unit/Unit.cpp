...
void Unit::SendPeriodicAuraLog(SpellPeriodicAuraLogInfo* pInfo)
{
    ...
    //for (var i = 0; i < count; ++i)
    {
        ...
        data << uint32(pInfo->overDamage > 0 ? pInfo->overDamage : -1);
        ...
        switch (aura->GetAuraType())
        {
            ...
            {
                data << uint32(aura->GetSpellInfo()->GetSchoolMask());  // SchoolMaskOrPower
            ...
            {
                data << uint32(0);                                      // SchoolMaskOrPower
                data << uint32(pInfo->absorb);                          // absorb
            ...
            {
                data << uint32(aura->GetMiscValue());                   // SchoolMaskOrPower
            ...
            {
                data << uint32(aura->GetMiscValue());                   // SchoolMaskOrPower
                data << float(pInfo->multiplier);                       // absorb or power gain
                data << uint32(0);                                      // resist
...
int32 Unit::CalculateSpellDamage(Unit const* target, SpellInfo const* spellProto, uint8 effect_index, int32 const* basePoints, Item* m_castitem, float* variance /*= nullptr*/) const
{
    if (variance)
        *variance = 0.0f;

    return spellProto->GetEffect(effect_index, GetSpawnMode()).CalcValue(this, basePoints, target, m_castitem, variance);
}
