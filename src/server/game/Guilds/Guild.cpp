...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
    ...
    ObjectGuid::LowType itemGuid = fields[18].GetUInt64();
...
///////////////////////////////////////////////////////////////////////////////
// Guild
Guild::Guild() : m_id(0), m_leaderGuid(), m_createdDate(0), m_accountsNumber(0), m_bankMoney(0), m_eventLog(nullptr),
...
bool Guild::Create(Player* pLeader, const std::string& name)
{
    ...
    sLog->outDebug(LOG_FILTER_GUILD, "GUILD: creating guild [%s] for leader %s (%u)",
        name.c_str(), pLeader->GetName(), m_leaderGuid.GetCounter());
...
void Guild::HandleRoster(WorldSession* session /*= nullptr*/)
{
    ...
    for (auto m : m_members)
    {
        ...
        memberData.AreaID = int32(onlineMember ? onlineMember->GetZoneId() : member->GetZoneId());
...
void Guild::HandleGuildRanks(WorldSession* session) const
{
    ...
    for (uint8 i = 0; i < _GetRanksSize(); ++i)
    {
        ...
        data << uint32(rankInfo->GetBankMoneyPerDay() / 10000);
...
void Guild::SendGuildChallengesInfo(WorldSession* session) const
{
    ...
    if (Member const* member = GetMember(session->GetPlayer()->GetGUID()))
    {
        ...
        for (uint8 i = 0; i < 6; ++i)
        {
            update.Gold[i] = gold[i];
            update.CurrentCount[i] = count[i];
            update.MaxCount[i] = maxCount[i];
            update.MaxLevelGold[i] = goldMaxLevel[i];
        }
...
void Guild::SendGuildChallengeComplete(WorldSession* session) const
{
    if (Member const* member = GetMember(session->GetPlayer()->GetGUID()))
    {
        ...
        completed.ChallengeType = 1;
        completed.CurrentCount = 1; 
        completed.MaxCount = 7;
        completed.GoldAwarded = 250;
        session->SendPacket(completed.Write());
    }
}
...
void Guild::BroadcastToGuild(WorldSession* session, bool officerOnly, const std::string& msg, uint32 language) const
{
    ...
    {
        WorldPacket data;
        ChatHandler::FillMessageData(&data, session, officerOnly ? CHAT_MSG_OFFICER : CHAT_MSG_GUILD, language, nullptr, ObjectGuid::Empty, msg.c_str(), nullptr);
        for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
...
void Guild::BroadcastAddonToGuild(WorldSession* session, bool officerOnly, const std::string& msg, const std::string& prefix) const
{
    ...
    {
        WorldPacket data;
        ChatHandler::FillMessageData(&data, session, officerOnly ? CHAT_MSG_OFFICER : CHAT_MSG_GUILD, LANG_ADDON, nullptr, ObjectGuid::Empty, msg.c_str(), nullptr, prefix.c_str());
        for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
            if (Player* player = itr->second->FindPlayer())
                if (player->GetSession() && _HasRankRight(player, officerOnly ? GR_RIGHT_OFFCHATLISTEN : GR_RIGHT_GCHATLISTEN) &&
                    !player->GetSocial()->HasIgnore(session->GetPlayer()->GetGUID()) &&
...
void Guild::GiveXP(uint32 xp, Player* source)
{
    ...
    while (GetExperience() >= sGuildMgr->GetXPForGuildLevel(GetLevel()) && GetLevel() < sWorld->getIntConfig(CONFIG_GUILD_MAX_LEVEL))
    {
        ...
        GetNewsLog().AddNewEvent(GUILD_NEWS_LEVEL_UP, time(nullptr), ObjectGuid::Empty, 0, _level);
        GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_REACH_GUILD_LEVEL, GetLevel(), 0, 0, nullptr, source);
...
void Guild::GuildNewsLog::AddNewEvent(GuildNews eventType, time_t date, ObjectGuid playerGuid, uint32 flags, uint32 data)
{
    uint32 id = GetNewId();
    ...
    stmt->setUInt64(0, GetGuild()->GetId());
    stmt->setUInt32(1, id);
    stmt->setUInt32(2, log.EventType);
    stmt->setUInt64(3, log.PlayerGuid.GetCounter());
