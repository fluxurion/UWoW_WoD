    ...
    struct boss_palehoofAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            if (instance)
            {
                ...
                Creature* temp = Unit::GetCreature((*me), instance->GetGuidData(DATA_MOB_FRENZIED_WORGEN));
                if (temp && !temp->isAlive())
                    temp->Respawn();

                temp = Unit::GetCreature((*me), instance->GetGuidData(DATA_MOB_FEROCIOUS_RHINO));
                if (temp && !temp->isAlive())
                    temp->Respawn();

                temp = Unit::GetCreature((*me), instance->GetGuidData(DATA_MOB_MASSIVE_JORMUNGAR));
                if (temp && !temp->isAlive())
                    temp->Respawn();

                temp = Unit::GetCreature((*me), instance->GetGuidData(DATA_MOB_RAVENOUS_FURBOLG));
                if (temp && !temp->isAlive())
                    temp->Respawn();
