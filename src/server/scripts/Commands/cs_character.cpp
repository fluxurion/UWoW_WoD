...
public:
    ...
    */
    static void HandleCharacterDeletedRestoreHelper(DeletedInfo const& delInfo, ChatHandler* handler)
    {
        ...
        if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
            sWorld->AddCharacterNameData(delInfo.Guid.GetCounter(), delInfo.name, (*result)[2].GetUInt8(), (*result)[0].GetUInt8(), (*result)[1].GetUInt8(), (*result)[2].GetUInt8());
    }
    ...
    */
    ...
    static bool HandlePDumpLoadCommand(ChatHandler* handler, char const* args)
    {
        ...
        switch (PlayerDumpReader().LoadDump(fileStr, accountId, name, guid.GetCounter()))
        {
            case DUMP_SUCCESS:
    ...
    static bool HandleCharacterGetrenameCommand(ChatHandler* handler, char const* args)
    {
        ...
        else 
        {
            ...
            {
                std::string PlayerNewName;
                sObjectMgr->GetPlayerNameByGUID(ObjectGuid::Create<HighGuid::Player>(result->Fetch()->GetUInt64()), PlayerNewName); // nom actuel en cas de rename multiple !
