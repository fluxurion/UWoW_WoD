            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        ...
                        {
                            events.ScheduleEvent(EVENT_1, 0.5 * IN_MILLISECONDS);
                            events.ScheduleEvent(EVENT_7 + urand(0, 2), 0.5 * IN_MILLISECONDS);
                            DoCast(SPELL_FIERY_BOULDER);
                            ++bouldersCounter;
                        } else
                        {
                            bouldersCounter = 0;
                            events.ScheduleEvent(EVENT_4, 0.5 * IN_MILLISECONDS);
                        }
                        break;
                    case EVENT_7: // {0, NPC_FIERY_BOULDER_RIGHT, DATA_BOULDER_R}
                        if (instance->GetData(DATA_BOULDER_R) != DONE)
                        {
                            ...
                            if (urand(0, 1))
                                events.ScheduleEvent(EVENT_8, 0.5 * IN_MILLISECONDS);
                            else
                                events.ScheduleEvent(EVENT_9, 0.5 * IN_MILLISECONDS);
                        break;
                    case EVENT_8: // {1, NPC_FIERY_BOULDER_LEFT, DATA_BOULDER_L}
                        if (instance->GetData(DATA_BOULDER_L) != DONE)
                        {
                            ...
                            if (urand(0, 1))
                                events.ScheduleEvent(EVENT_7, 0.5 * IN_MILLISECONDS);
                            else
                                events.ScheduleEvent(EVENT_9, 0.5 * IN_MILLISECONDS);
                        break;
...
public:
    ...
    struct npc_fiery_boulder_rightAI : public ScriptedAI
    {
        ...
        void DoAction(int32 const action) override
        {
            ...
            {
                case ACTION_1:
                    events.ScheduleEvent(EVENT_1, 0.1 * IN_MILLISECONDS);
                    break;
                default:
                    break;
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                    {
                        events.ScheduleEvent(EVENT_2, 1.5 * IN_MILLISECONDS);
