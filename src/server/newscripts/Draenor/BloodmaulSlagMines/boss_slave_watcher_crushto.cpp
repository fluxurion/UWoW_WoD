...
public:
    ...
    struct boss_slave_watcher_crushtoAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            events.ScheduleEvent(EVENT_1, 11 * IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_2, urand(14, 26) * IN_MILLISECONDS);
            events.ScheduleEvent(EVENT_3, urand(3, 8) * IN_MILLISECONDS);
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                {
                    case EVENT_1:
                        events.ScheduleEvent(EVENT_1, urand(15, 25) * IN_MILLISECONDS);
                    ...
                    case EVENT_4:
                    {
                        events.ScheduleEvent(EVENT_4, urand(15, 21) * IN_MILLISECONDS);
                        float x = 0.0f, y = 0.0f;
    ...
    struct npc_captured_minerAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                {
                    case EVENT_5:
                        events.ScheduleEvent(EVENT_5, urand(2, 5) * IN_MILLISECONDS);
