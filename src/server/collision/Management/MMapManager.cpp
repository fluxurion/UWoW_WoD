...
namespace MMAP
{
    ...
    PhasedTile* MMapManager::LoadTile(uint32 mapId, int32 x, int32 y)
    {
        ...
        {
            sLog->outError(LOG_FILTER_MAPS, "MMAP:LoadTile: Bad header in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            delete pTile;
            return nullptr;
        }
        ...
        {
            sLog->outError(LOG_FILTER_MAPS, "MMAP:LoadTile: %04u%02i%02i.mmtile was built with generator v%i, expected v%i",
        ...
        if (!result)
        {
            sLog->outError(LOG_FILTER_MAPS, "MMAP:LoadTile: Bad header or data in mmap %04u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            delete pTile;
            return nullptr;
        }
    ...
    {
        sLog->outDebug(LOG_FILTER_MAPS, "MMAP:LoadPhaseTiles: Loading phased mmtiles for map %u, x: %i, y: %i", phasedMapData->first, x, y);
        ...
        for (uint32 phaseMapId : phasedMapData->second)
        {
            ...
            {
                sLog->outDebug(LOG_FILTER_MAPS, "MMAP:LoadPhaseTiles: Loaded phased %04u%02i%02i.mmtile for root phase map %u", phaseMapId, x, y, phasedMapData->first);
    ...
    {
        sLog->outDebug(LOG_FILTER_MAPS, "MMAP:UnloadPhaseTile: Unloading phased mmtile for map %u, x: %i, y: %i", phasedMapData->first, x, y);
        ...
        for (uint32 phaseMapId : phasedMapData->second)
        {
            ...
            {
                sLog->outDebug(LOG_FILTER_MAPS, "MMAP:UnloadPhaseTile: Unloaded phased %04u%02i%02i.mmtile for root phase map %u", phaseMapId, x, y, phasedMapData->first);
    ...
    void MMapData::RemoveSwap(PhasedTile* ptile, uint32 swap, uint32 packedXY)
    {
        ...
        if (dtStatusFailed(navMesh->removeTile(loadedTileRefs[packedXY], NULL, NULL)))
            sLog->outError(LOG_FILTER_MAPS, "MMapData::RemoveSwap: Could not unload phased %04u%02i%02i.mmtile from navmesh", swap, x, y);
        else
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMapData::RemoveSwap: Unloaded phased %04u%02i%02i.mmtile from navmesh", swap, x, y);
            ...
            if (dtStatusSucceed(navMesh->addTile(_baseTiles[packedXY]->data, _baseTiles[packedXY]->dataSize, 0, 0, &loadedTileRefs[packedXY])))
            {
                ...
                sLog->outError(LOG_FILTER_MAPS, "MMapData::RemoveSwap: Could not load base %04u%02i%02i.mmtile to navmesh", _mapId, x, y);
        }
    ...
    void MMapData::AddSwap(PhasedTile* ptile, uint32 swap, uint32 packedXY)
    {
        ...
        {
            sLog->outError(LOG_FILTER_MAPS, "MMapData::AddSwap: Could not unload %04u%02i%02i.mmtile from navmesh", _mapId, x, y);
            delete pt;
        }
        else
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMapData::AddSwap: Unloaded %04u%02i%02i.mmtile from navmesh", _mapId, x, y);
            ...
            if (dtStatusSucceed(navMesh->addTile(ptile->data, ptile->fileHeader.size, 0, 0, &loadedTileRefs[packedXY])))
            {
                ...
                sLog->outError(LOG_FILTER_MAPS, "MMapData::AddSwap: Could not load %04u%02i%02i.mmtile to navmesh", swap, x, y);
        }
    }
